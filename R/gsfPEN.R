`gsfPEN` <- function(
    summaryZ, Nvec, plinkLD, NumIter = 1000, RupperVal = NULL, breaking = 1,
    ...
    if (any(c(is.null(tuningMatrix), is.null(lambdavec_func)))) {
        ...
        output <- Tuning_setup_group_func(
            lambdavec_func, lambdavec_func_limit_len, p.Threshold, numfunc, tauvec, subtuning, Lambda_limit,
    ...
    # dims2[8] = ncol_AllTuningMatrix
    Z <- .C("gsfFunc",
        as.double(t(summaryBetas)),
        as.integer(ldJ),
        as.integer(dims),
        Numitervec = as.integer(Numitervec),
        as.integer(t(IndexMatrix)),
        as.integer(IndJ),
        as.double(ldvec),
        as.integer(ChrIndexBeta),
        as.double(RupperVal),
        as.double(Init_summaryBetas),
        as.double(t(SDvec)),
        as.double(t(tuningMatrix)),
        BetaMatrix = as.double(t(BetaMatrix)),
        penalty,
        as.double(lambda0vec),
        as.double(t(Zmatrix)),
        as.integer(dims2),
        AllTuningMatrix = as.double(t(AllTuningMatrix)),
        as.double(lambdavec_func),
        as.integer(t(funcLambda)),
        as.integer(IfuncSNP),
        PACKAGE = "PANPRS"
    )
